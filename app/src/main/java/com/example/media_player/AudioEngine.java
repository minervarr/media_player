            ...
            while (!stopped) {
                ...
                try {
                    ...
                    for (int i = 0; i < blockSize; i += 4) {
                        packed[packedPos++] = leftBlock[i];
                        packed[packedPos++] = leftBlock[i + 1];
                        packed[packedPos++] = leftBlock[i + 2];
                        packed[packedPos++] = leftBlock[i + 3];
                        if (channelCount >= 2) {
                            packed[packedPos++] = rightBlock[i];
                            packed[packedPos++] = rightBlock[i + 1];
                            packed[packedPos++] = rightBlock[i + 2];
                            packed[packedPos++] = rightBlock[i + 3];
                        }
                    }
        ...
        decodeThread = new Thread(() -> {
            ...
            while (!stopped && !outputDone) {
                ...
                if (endOfStream && !seeking) {
                    ...
                    try {
                        gaplessDecoder.processFrame(pcmData, 0, pcmData.length);
                    } catch (InterruptedException e) {
                        break;
                    }
                }
    ...
    public void queueNext(Context context, Uri uri) {
        ...
        new Thread(() -> {
            try {
                ...
                NativePcmBuffer buf = new NativePcmBuffer(PCM_BUFFER_SIZE);
                if (nextStopped) { cod.stop(); cod.release(); ext.release(); return; }
